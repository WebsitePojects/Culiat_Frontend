...
const WebsiteFeedback = () => {
  ...
  const [activeTab, setActiveTab] = useState("website"); // "website" or "committee"
  ...
  }, [activeTab, currentPage, selectedStatus, selectedRating, dateFrom, dateTo, selectedCommittee]);
  ...
  const fetchCommitteeMessages = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem("token");
      const params = new URLSearchParams({
      ...
      if (selectedStatus !== "all") params.append("status", selectedStatus);
      if (selectedCommittee !== "all") params.append("committeeId", selectedCommittee);
      if (dateFrom) params.append("dateFrom", dateFrom);
      if (dateTo) params.append("dateTo", dateTo);
      if (searchQuery) params.append("search", searchQuery);

      const response = await axios.get(
        `${API_URL}/api/committee-messages?${params.toString()}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      showError("Failed to load committee messages");
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchCommitteeStats = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get(
        `${API_URL}/api/committee-messages/stats`,
        { headers: { Authorization: `Bearer ${token}` } }
      );

      if (response.data.success) {
        setCommitteeStats(response.data.data);
      }
    } catch (error) {
  ...
  const handleMarkAsRead = async (id) => {
    try {
      const token = localStorage.getItem("token");
      const endpoint = activeTab === "website" ? `${API_URL}/api/contact-messages/${id}/status` : `${API_URL}/api/committee-messages/${id}/status`;
      await axios.put(
        endpoint,
        { status: "read" },
        { headers: { Authorization: `Bearer ${token}` } }
  ...
  const confirmMarkAsSpam = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const endpoint = activeTab === "website" ? `${API_URL}/api/contact-messages/${actionFeedback._id}/spam` : `${API_URL}/api/committee-messages/${actionFeedback._id}/spam`;
      await axios.put(
        endpoint,
        { isSpam: true },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      showSuccess("Message marked as spam");
      if (activeTab === "website") {
        fetchFeedbacks();
        fetchStats();
      } else {
  ...
  const confirmBlockIP = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.put(
        `${API_URL}/api/contact-messages/block-ip`, // IP blocking is currently only for website feedback
        { ipAddress: actionIP },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      showSuccess(`IP ${actionIP} has been blocked`);
      fetchFeedbacks(); // Only affects website feedback
      fetchStats();
    } catch (error) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const endpoint = activeTab === "website" ? `${API_URL}/api/contact-messages/${actionFeedback._id}` : `${API_URL}/api/committee-messages/${actionFeedback._id}`;
  ...
  const currentStats = activeTab === "website" ? {
    total: stats.total,
    new: stats.new,
    avg: stats.avgRating,
    extra: stats.blockedIPs,
    extraLabel: "Blocked IPs",
    extraIcon: <Ban className="w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-5 md:h-5 text-red-400" />
  } : {
    total: committeeStats.total,
    new: committeeStats.newCount,
    avg: null,
    extra: committeeStats.statsByCommittee?.length || 0,
    extraLabel: "Committees",
    extraIcon: <Shield className="w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-5 md:h-5 text-red-400" />
  };
        ...
        <div className="relative z-10">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-4 sm:mb-6">
            ...
            <div className="flex items-center gap-1 mb-6 p-1 bg-white/5 rounded-xl w-fit">
              ...
              <button
                onClick={() => { setActiveTab("committee"); setCurrentPage(1); }}
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === "committee"
                  ? "bg-white text-purple-900 shadow-lg"
                  : "text-white/60 hover:text-white hover:bg-white/10"
                  }`}
              >
              ...
              {activeTab === "website" && (
                <div className="bg-white/10 backdrop-blur-sm rounded-lg sm:rounded-xl p-2.5 sm:p-3 md:p-4 border border-white/10">
                  <div className="flex items-center gap-2 sm:gap-3">
                    ...
                    <div>
                      <p className="text-base sm:text-lg md:text-xl font-bold text-white">
                        {currentStats.avg > 0 ? currentStats.avg.toFixed(1) : "0.0"}
                      </p>
                      <p className="text-[10px] sm:text-xs text-purple-200/70">
                        Avg. Rating
                      </p>
                ...
                {feedbacks.map((feedback) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <div
                          className={`w-8 h-8 rounded-full flex items-center justify-center ${feedback.isRegistered
                            ? "bg-blue-100 dark:bg-blue-900/30"
                            : "bg-gray-100 dark:bg-gray-700"
                            }`}
                        >
                          <User
                            className={`w-4 h-4 ${feedback.isRegistered
                  ...
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-shrink-0">
                        <div
                          className={`w-10 h-10 rounded-full flex items-center justify-center ${feedback.isRegistered
                            ? "bg-blue-100 dark:bg-blue-900/30"
                            : "bg-gray-100 dark:bg-gray-700"
                            }`}
                        >
                          <User
                            className={`w-5 h-5 ${feedback.isRegistered
                              ? "text-blue-600 dark:text-blue-400"
                              : "text-gray-500 dark:text-gray-400"
                              }`}
                          />
          ...
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
            ...
            <div className="relative bg-white dark:bg-gray-800 rounded-t-2xl sm:rounded-xl shadow-xl w-full sm:max-w-lg max-h-[90vh] overflow-y-auto animate-in slide-in-from-bottom sm:zoom-in-95 duration-200 p-4 sm:p-6">
              ...
              <div className="space-y-3 sm:space-y-4">
                <div className="flex items-center gap-2 sm:gap-3">
                  <div
                    className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center ${selectedFeedback.isRegistered
                      ? "bg-blue-100 dark:bg-blue-900/30"
                      : "bg-gray-100 dark:bg-gray-700"
                      }`}
                  >
                    <User
                      className={`w-5 h-5 sm:w-6 sm:h-6 ${selectedFeedback.isRegistered
                        ? "text-blue-600 dark:text-blue-400"
                        : "text-gray-500 dark:text-gray-400"
                        }`}
                    />
